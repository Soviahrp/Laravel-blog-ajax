...
class ArticleController extends Controller
{
    ...
    {
        try {
            $this->articleService->toggleConfirmation($uuid);

            return response()->json([
                'status' => 'success',
                'message' => 'Article confirmation status updated successfully',
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
                'message' => 'Failed to update article confirmation status',
            ], 500);
        }
    }
